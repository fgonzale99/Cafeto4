...
class PaymentList extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      error: '',
      elementsList: [],
      moduleData: { name: 'evaluacionesPagos', id: this.props.match.params.id },
      modalIsOpen: false,
    ...
    this.paymentEnable = async (id) => {

      //this.setState({ modalIsOpen: true});
      var yes = confirm("Confirme si desea habilitar el pago.");
      if (yes === true){

        const res = await reviewServices.paymentEnable(id);

        if (res.success) {
          var redirect = '/process/payment/list/' + this.props.match.params.id;
          window.location.href = redirect;
        }
        else{
          alert(res.message);
    ...
    this.paymentsAvailable = async (id) => {

      var yes = confirm("Confirme que desea habilitar los pagos del proceso actual.");
      if (yes === true){

        const res = await processServices.paymentAvailable(this.props.match.params.id);
    ...
    this.onChangeFilter = async (e) => {
        ...
        const page = 1;
        const rest = await reviewServices.processReviews(this.state.moduleData, dataFilter, page);
  ...
  async componentDidMount() {
    try{
        ...
        if(moduleData.success == true){
            ...
            const res = await reviewServices.processReviews(this.state.moduleData, [], []);
  ...
  render() {
    ...
    const data = {
      header: {
        ...
        functions: { paymentsAvailable: this.paymentsAvailable }
      },
      body: cardBody,
      footer: paginator,
    };
