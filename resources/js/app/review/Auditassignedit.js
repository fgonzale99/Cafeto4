...
class Editassign extends React.Component {
  ...
  async componentDidMount(){
    try{
        ...
        if(moduleData.success == true){
          ...
          const res = await reviewServices.getSummary(this.props.match.params.id);
  ...
  async saveEdit(e) {
    e.preventDefault()
      ...
      const res = await reviewServices.auditAssignUpdate(data, this.state.moduleData);

      if (res.success) {
        var redirect = '/review/board/' + this.state.moduleData.values.review + '?message=updated';
