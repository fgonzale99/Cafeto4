...
class Summary extends React.Component {
  ...
  render() {
    ...
    var answerContent = [];
    var answerReview = Object.values(this.state.resReview.form);
    answerReview.map((page,i)=>{
        page.map((item,j)=>{
          answerContent.push(<h2>{ item.pageName }</h2>);
          item.questions.map((itemAnswer,k)=>{
            answerContent.push(Parser(itemAnswer.label));
            answerContent.push(<br/>);
            if(itemAnswer.value) answerContent.push(Parser(itemAnswer.value));
            answerContent.push(<br/>);
          });
        });
    });
