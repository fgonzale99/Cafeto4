...
class List extends React.Component {
  ...
  constructor(props) {
    ...
    this.onDuplicate = async (id) => {

      //this.setState({ modalIsOpen: true});
   
      var yes = confirm("Confirme si desea duplicar el elemento.");
      if (yes === true){

        const res = await crudServices.duplicate(id, this.state.moduleData);

        if (res.success) {
          var redirect = '/crud/'+ this.state.moduleData.name +'/list?message=duplicate ';
          window.location.href = redirect;
        }
        else{
          alert(res.message);
    ...
    this.onChangeFilter = async (e) => {
        ...
        if(form){
          ...
          const page = 1;
          const rest = await crudServices.list(this.state.moduleData, dataFilter, page, this.state.moduleData.prefilter);
  ...
  async componentDidMount() {
    try{
        ...
        if(moduleData.success == true){
            ...
            const res = await crudServices.list(this.state.moduleData, [], 0, this.state.moduleData.prefilter);
