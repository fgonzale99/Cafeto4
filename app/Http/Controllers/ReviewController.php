...
*/
  ...
  public function ethicCodeQR(Request $request)
  {
   ...
   if (!empty($_SERVER['HTTP_CLIENT_IP'])) {
    ...
    $fechaFirma = $review->assignDate;
    $hostName=request()->getHost(); 
    $codigoFirma = 'http://'.$hostName.'/review/verifyQrCode/' . md5('firmaDatos') . '/' . $review->id . '/' . md5($fechaFirma) . '/' . $review->getReviewer->id . '/' . $ip;
    ...
    $arrayValues['idCodigoEtica'] = $review->id;
    $arrayValues['fechaFirma'] = $review->assignDate;
    $arrayValues['firmaCodigo'] = '<img src="data:image/svg;base64,' . $imagenFirma . '" widht="80" height="80"/>';
    $arrayValues['nombreEvaluador'] = $review->getReviewer->name;
    $arrayValues['identificacion'] = $review->getReviewer->document;
    $arrayValues['tipoId'] = $review->getReviewer->documentType;
